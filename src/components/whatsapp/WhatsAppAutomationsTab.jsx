...
export default function WhatsAppAutomationsTab({ coachEmail }) {
  ...
  const { data: rules = [], isLoading: loadingRules } = useQuery({
    queryKey: ['whatsappRules', coachEmail],
    queryFn: () => base44.entities.WhatsAppAutomationRule.filter({ coach_email: coachEmail }),
    enabled: !!coachEmail,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      if (editing?.id) return base44.entities.WhatsAppAutomationRule.update(editing.id, data);
      return base44.entities.WhatsAppAutomationRule.create({ ...data, coach_email: coachEmail });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['whatsappRules', coachEmail] });
      setShowDialog(false);
      toast.success('כלל נשמר');
    },
    onError: (e) => toast.error('שגיאה: ' + e.message),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.WhatsAppAutomationRule.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['whatsappRules', coachEmail] }),
  });

  const toggleMutation = useMutation({
    mutationFn: ({ id, is_active }) => base44.entities.WhatsAppAutomationRule.update(id, { is_active }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['whatsappRules', coachEmail] }),
  });
