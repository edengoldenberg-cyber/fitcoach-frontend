...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { coachEmail, toPhoneE164: toPhoneRaw, text, toName, contextType, contextId, templateKey, leadId: payloadLeadId, opt_out_verified } = body;
    ...
    // ── LEAD OPT-OUT: re-verify from DB regardless of what the client claims ──
    // The client passes opt_out_verified as a hint, but we always re-read from DB
    // so that the frontend filter cannot be bypassed.
    if (resolvedLeadId) {
      try {
        ...
        if (lead) {
          if (lead.waOptOut === true) {
            console.log(`[OPT_OUT] BLOCKED lead ${resolvedLeadId} — waOptOut=true`);
            return Response.json({
              ok: false,
              blocked: true,
              errorCode: 'LEAD_OPT_OUT',
              message: 'Lead has opted out of WhatsApp messages'
            }, { status: 200 });
          }
          if (lead.whatsapp_notifications_enabled === false) {
            console.log(`[OPT_OUT] BLOCKED lead ${resolvedLeadId} — whatsapp_notifications_enabled=false`);
            return Response.json({
              ok: false,
              blocked: true,
              errorCode: 'LEAD_NOTIFICATIONS_DISABLED',
              message: 'WhatsApp notifications are disabled for this lead'
            }, { status: 200 });
          }
        }
      } catch (optOutErr) {
        // Non-blocking: if DB read fails, log and continue — fail open (prefer delivery over silence).
        console.log('[OPT_OUT] Lead opt-out check failed (non-blocking):', optOutErr.message);
      }
    }
